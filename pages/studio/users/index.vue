...
const { result: countResult, refetch: reloadCount } = await useAsyncQuery(StudioCountUsersDocument, vars, {
  debounce: 500
})
...
const { result, fetchMore: refeachUsers } = useQuery(StudioGetUsersDocument, {
  filter: filter.value
})
...
watchDebounced(() => vars.value.filter.name, (val, oldValue) => {
  if (val !== oldValue) {
